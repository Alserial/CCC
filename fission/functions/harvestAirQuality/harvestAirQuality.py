...
def main():
    ...
    """
    response = requests.get(f"{Utils.config('MRC_OBJ_STORE_URL')}/All_SA2.json")
    ALL_SA2 = response.json()
    User_Agent = 'curl/8.4.0'
    vic_url = Utils.config("AIR_VIC_URL")
    params = {"environmentalSegment": "air"}
    headers = {'User-Agent': User_Agent,
               'Cache-Control': 'no-cache',
               'X-API-key': Utils.secret("air_vic_apikey")}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for row in data_mel.get('records'):
                ...
                if site_health_advices is not None and isinstance(site_health_advices, list) and len(site_health_advices) > 0:
                    ...
                    sa2_code, sa2_name = find_sa2(lon, lat, ALL_SA2)
                    start_time = air_detail.get("since")
                    site_name = row.get("siteName")
                    _id = f"{site_name}_{start_time}"
                    observation_data = {
                        "_index": "air", 
                        "_id": _id, 
        ...
        site_details_url = f"{Utils.config('AIR_NSW_URL')}/Data/get_SiteDetails"
        ...
        observations_url = f"{Utils.config('AIR_NSW_URL')}/Data/get_Observations"
        ...
        for observation in observations_data:
            ...
            site_id = observation["Site_Id"]
            site_details = site_id_to_details.get(site_id, {})
            hour_description = observation['HourDescription'].replace(" ", "")
            _id = f'{site_details.get("SiteName")}-{observation["Date"]}{hour_description}'
        ...
        response = bulk(client, merged_data)
